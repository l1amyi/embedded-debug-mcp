...
5. **危险操作要保守**：会擦写目标 flash 的操作（`flash` / `erase`）在未获用户明确同意前不要在真机上执行。
6. **文档与代码一致**：改了行为就同步 `README.md` 与本文件的实测数据，不要留下过时结论。
7. **注释解释「为什么」**，尤其是那些看起来多余、实际是为绕开上述陷阱的代码 —— 否则后来者（包括你自己）会把它们「优化」掉。
